...
ROWS: list[tuple[str, str, str, str, str, str]] = [
    ...
    ("INS-1","Install","one-line install + overrides","OYSTER_REPO/BRANCH/HOME","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-2","Install","platform detection","macos/linux/refuse","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-3","Install","sudo/root handling","sudo or die","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-4","Install","Homebrew requirement","die if absent (mac)","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-5","Install","ensure git","install if missing","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-6","Install","fresh clone","branch --depth 1","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-7","Install","update in place","fetch+checkout FETCH_HEAD","PASS (fixed)","branch-trap fixed; code"),
    ("INS-8","Install","ensure Python","install if missing","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-9","Install","ensure PostgreSQL","install if missing","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-10","Install","start DB + wait","pg_isready loop or die","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-11","Install","DB login role (linux)","create role for OS user","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-12","Install","create db idempotent","untouched if exists","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-13","Install","core venv + deps","venv + requirements","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-14","Install","studio venv + deps","venv-studio","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-15","Install",".env creation","defaults, 0600, never clobber","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-16","Install","migrate","main.py migrate","PASS","migrate runs"),
    ("INS-17","Install","attach MCP to hosts","claude/codex mcp add","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-18","Install","desktop launcher","double-click icon","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-19","Install","final guidance","next steps printed","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
    ("INS-20","Install","auto-open Studio","if signed in","PASS","live: install.sh run from scratch 2026-08-07 — clone→venvs→deps→migrate→MCP attach, EXIT=0, imports ok"),
