...
STORIES = [
    # --- Launch & setup ---
    ("L1", "Launch/Setup", "I run the one-line installer", "clones branch, installs Python+Postgres if missing, creates+migrates DB, attaches Oyster; idempotent", "REVIEWED", "Branch-trap defect found + fixed by inspection (header defaulted to main; added the OYSTER_BRANCH example). The end-to-end from-scratch install is NOT executed — it installs system packages and would modify this host.", "Code review only — not run from scratch (would modify the host)."),
    ...
    ("C6", "Chat", "I send while a run is in progress", "no hang/duplicate; defined behavior", "NOTE", "Today: input is disabled during a run, and the send endpoint returns a clear 409 as a backstop. Chat-while-working is the planned v1.1 upgrade.", "409-on-busy by code (studio.py:489) + input-disable in send(); not re-driven via an authed send this pass."),
    # --- Live activity ---
    ...
    # --- Report & deliverables ---
    ("R1", "Report", "I open the Report/doc view after a run", "designed HTML renders inline; per-job dropdown", "PASS", "POV/at-a-glance/[n] all render.", "Playwright 2026-08-07: job 1861 rendered in the doc view — 'The wedge is trust, not texture', at-a-glance + [n] present, 0 console errors."),
    ...
    ("R4", "Report", "I open deck.pptx", "16:9 deck: title, POV, stat tiles, charts, recs, sources", "PASS", "9-slide deck built.", "Grader 2026-08-07: deck.pptx opens with 9 slides."),
    ...
    ("P1", "Control Center", "I use the 8765 dashboard (Gaming Pulse + capture pairing)", "separate legacy app; runs the gaming pipeline + browser-capture pairing", "NOTE", "Distinct from the Studio (8770). Legacy 'two products in one repo' — works, but out of the Studio demo path.", "Not exercised this pass — out of the Studio demo path."),
...
def build(path: Path) -> None:
    ...
    legend = [
        ...
        "test, a Playwright drive, or code review only. The anchor is a fresh end-to-end run through",
        "the real UI: a Ninja Creami competitor brief → finished report (job 1861). That run also",
        "surfaced 3 real defects, now fixed: E5 (report jargon page), R3/R7 (gaming language in",
        "reports), UX1 (409s when opening a mid-run report).",
    ]
